...
func (d *DB) CreateBoard(ctx context.Context, create *store.Board) (*store.Board, error) {
	...
	stmt := "INSERT INTO `board` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	...
	id := int32(rawID)
	board, err := d.ListBoards(ctx, &store.FindBoard{ID: &id})
	if err != nil {
		return nil, err
	}
	if len(board) == 0 {
		return nil, errors.Errorf("failed to create board")
	}
	return board[0], nil
}
...
func (d *DB) ListBoards(ctx context.Context, find *store.FindBoard) ([]*store.Board, error) {
	...
	query := "SELECT `board`.`id`, `board`.`uid`, `board`.`creator_id`, UNIX_TIMESTAMP(`board`.`created_ts`) AS `created_ts`, UNIX_TIMESTAMP(`board`.`updated_ts`) AS `updated_ts`, `board`.`title`, `board`.`description` FROM `board` WHERE " +
...
func (d *DB) CreateBoardColumn(ctx context.Context, create *store.BoardColumn) (*store.BoardColumn, error) {
	...
	stmt := "INSERT INTO `board_column` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	result, err := d.db.ExecContext(ctx, stmt, args...)
	if err != nil {
		return nil, err
	}
	...
	id := int32(rawID)
	columns, err := d.ListBoardColumns(ctx, &store.FindBoardColumn{ID: &id})
	if err != nil {
		return nil, err
	}
	if len(columns) == 0 {
		return nil, errors.Errorf("failed to create board column")
	}
...
func (d *DB) ListBoardColumns(ctx context.Context, find *store.FindBoardColumn) ([]*store.BoardColumn, error) {
	...
	query := "SELECT `board_column`.`id`, `board_column`.`uid`, `board_column`.`board_id`, UNIX_TIMESTAMP(`board_column`.`created_ts`) AS `created_ts`, UNIX_TIMESTAMP(`board_column`.`updated_ts`) AS `updated_ts`, `board_column`.`title`, `board_column`.`order` FROM `board_column` WHERE " +
