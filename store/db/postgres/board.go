...
func (d *DB) CreateBoard(ctx context.Context, create *store.Board) (*store.Board, error) {
	...
	stmt := "INSERT INTO board (" + strings.Join(fields, ", ") + ") VALUES (" + placeholders(len(args)) + ") RETURNING id, created_ts, updated_ts"
...
func (d *DB) ListBoards(ctx context.Context, find *store.FindBoard) ([]*store.Board, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "board.id = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "board.uid = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := find.CreatorID; v != nil {
		where, args = append(where, "board.creator_id = "+placeholder(len(args)+1)), append(args, *v)
	}
...
func (d *DB) UpdateBoard(ctx context.Context, update *store.UpdateBoard) error {
	...
	stmt := "UPDATE board SET " + strings.Join(set, ", ") + " WHERE id = " + placeholder(len(args))
...
func (d *DB) DeleteBoard(ctx context.Context, delete *store.DeleteBoard) error {
	if _, err := d.db.ExecContext(ctx, "DELETE FROM board WHERE id = "+placeholder(1), delete.ID); err != nil {
...
func (d *DB) CreateBoardColumn(ctx context.Context, create *store.BoardColumn) (*store.BoardColumn, error) {
	...
	stmt := "INSERT INTO board_column (" + strings.Join(fields, ", ") + ") VALUES (" + placeholders(len(args)) + ") RETURNING id, created_ts, updated_ts"
	if err := d.db.QueryRowContext(ctx, stmt, args...).Scan(&create.ID, &create.CreatedTs, &create.UpdatedTs); err != nil {
		return nil, err
	}
	return create, nil
...
func (d *DB) ListBoardColumns(ctx context.Context, find *store.FindBoardColumn) ([]*store.BoardColumn, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "board_column.id = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "board_column.uid = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := find.BoardID; v != nil {
		where, args = append(where, "board_column.board_id = "+placeholder(len(args)+1)), append(args, *v)
	}
...
func (d *DB) UpdateBoardColumn(ctx context.Context, update *store.UpdateBoardColumn) error {
	...
	args = append(args, update.ID)
	stmt := "UPDATE board_column SET " + strings.Join(set, ", ") + " WHERE id = " + placeholder(len(args))
	if _, err := d.db.ExecContext(ctx, stmt, args...); err != nil {
...
func (d *DB) DeleteBoardColumn(ctx context.Context, delete *store.DeleteBoardColumn) error {
	if _, err := d.db.ExecContext(ctx, "DELETE FROM board_column WHERE id = "+placeholder(1), delete.ID); err != nil {
